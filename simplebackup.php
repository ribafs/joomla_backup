...
$db_file = JURI::root().'backup'.DS.$database.'_'.$date.'.sql';
...
function backup_tables($dbhost,$dbuser,$dbpass,$database,$db_file)
{
        ...
        //save file
        $file='..'.DS.'backup'.DS.$database.'_'.$date.'.sql';
...
$bypass3 = JPATH_SITE.DS.JRequest::getVar('bypass3');

function Zip($source, $destination, $bypass1, $bypass2, $bypass3)
{
...
Zip("..".DS, $portal2, $bypass1, $bypass2, $bypass3);
backup_tables($dbhost,$dbuser,$dbpass,$database,$db_file);
	
JFactory::getApplication()->enqueueMessage( JText::_('COM_SIMPLEBACKUP_SUCCESS'),'message');
$backup = JURI::root().'backup';
?>
<h3>Downloads</h3>
